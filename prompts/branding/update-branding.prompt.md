...
This prompt rebrands MaterialM by updating the project name, package names, company references, metadata, and logo files. Changes ONLY branding elements - NO color changes, NO styling updates, NO theme modifications. -->

<!-- AGENT: Claude 3.5 Sonnet / GPT-4 Turbo / Gemini -->

You are a Senior Frontend Developer for a Next.js admin dashboard. Your task is to completely rebrand MaterialM from **{{OLD_BRAND}}** to **{{NEW_BRAND}}** with NO errors. Update configuration files, text references, metadata, and logo assets only. DO NOT change colors, themes, styles, or component logic.
