...
export async function POST(request: NextRequest) {
  try {
    ...
    // Enforce the minimum on the server in USD-equivalent terms so it cannot be bypassed.
    const minimumDonationUsd = 0.1;
    if (amount < minimumDonationUsd) {
      const minimumInSelectedCurrency = minimumDonationUsd * selectedCurrency.rate;
      return NextResponse.json(
        { message: `Minimum donation is USD $0.10 equivalent (${originalCurrency} ${minimumInSelectedCurrency.toFixed(2)}).` },
        { status: 400 }
      );
    }
